...
class Connection {
 ...
 public:
  ...
  //
  // The file name is always 8 bits since we want to use the 8-bit version of
  // sqlite3_open. The string can also be sqlite's special ":memory:" string.
  bool Open(const char* path);

  // See Open for information.
  ...
  // See Execute above for information.
  ...
  // See GetCachedStatement above for information.
  scoped_refptr<StatementRef> GetCachedStatement(const StatementID& id,
                                                 const std::string& sql) {
    return GetCachedStatement(id, sql.c_str());
  }
  ...
  // See GetUniqueStatement above for information.
  scoped_refptr<StatementRef> GetUniqueStatement(const std::string& sql) {
    return GetUniqueStatement(sql.c_str());
  }

  // Backup --------------------------------------------------------------------

  // Returns if backing up the database was successful
  bool BackupDatabaseTo(const char* src_db, Connection& conn,
      const char* dest_db) const;

  // See BackupDatabaseTo above for information.
  bool BackupDatabaseTo(const std::string& src_db, Connection& conn,
      const std::string& dest_db) const {
    return BackupDatabaseTo(src_db.c_str(), conn, dest_db.c_str());
  }

  // Returns if backing up all but the temporary database was successful
  bool BackupTo(Connection& conn) const;

  // Returns if backing up the temporary database was successful
  bool BackupTemporaryTo(Connection& conn) const;
  ...
  // See DoesTableExist for information.
  bool DoesTableExist(const std::string& table_name) const {
    return DoesTableExist(table_name.c_str());
  }
